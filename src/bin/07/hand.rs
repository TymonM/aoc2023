...
impl Hand {
    pub fn from(cards: &str, bet: &str, joker: bool) -> Hand {
        let mut hand = Hand{cards: [Card::new(); 5], bet: bet.parse().unwrap()};
        let mut i = 0;
        while i < 5 {
    ...
    pub fn get_power(&self) -> usize {
        ...
        for card in self.cards {
            if card.get_value() == 0 {jokers += 1; continue}
        ...
        return doubles; // nasty, covers remaining cases
    }
}
...
impl Card {
    ...
    pub fn set_value(&mut self, symbol: char, joker: bool) {
        match symbol {
            ...
            'K' => self.value = 12,
            'A' => self.value = 13,
            num => self.value = num.to_digit(10).expect(&format!("Unknown symbol `{}`", num)) as usize - 1,
        }
    }
