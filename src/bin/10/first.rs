...
pub fn run(input: &str) {
    ...
    let (mut forward_direction, mut back_direction) = Pipe::extrapolate_start(&mut pipe_map, starting_coords);
    ...
    loop {
        ...
        forward_direction = pipe_map[forward_coords.1][forward_coords.0].clone().unwrap()
            .other(forward_direction.opposite()).unwrap();
        ...
        back_direction = pipe_map[back_coords.1][back_coords.0].clone().unwrap()
            .other(back_direction.opposite()).unwrap();
    }
